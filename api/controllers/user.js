...
exports.signup = (req, res, next) => {
    // encrypte le mot de passe contenu dans la requette
    bcrypt.hash(req.body.password, 10)
      // si tout se passe bien
      .then(hash => {
        // crée un nouvel utilisateur
        const user = new User({
            // a partire de l'email contenu dans la requet
        ...
        user.save()
          // si tout se passe birn, envoi un message de succès (obligatoire)
          .then(() => res.status(201).json({ message: 'Utilisateur créé !' }))
          // en cas 'derreure, envoie l'erreure
